...
impl Backend for KrunvmBackend {
    async fn create(&self, vm: &VmInstance) -> Result<()> {
        ...
        for (host_port, guest_port) in &vm.spec.ports {
            cmd.arg("--port").arg(format!("{}:{}", host_port, guest_port));
        }
        
        for (host_path, guest_path) in &vm.spec.volumes {
            cmd.arg("-v").arg(format!("{}:{}", host_path.display(), guest_path.display()));
        }
    ...
    async fn attach(&self, vm: &VmInstance) -> Result<()> {
        ...
        let mut cmd = Self::krunvm_command();
        cmd.args(&["start", &vm.id, "--", "sh", "-c", &format!("export TERM=vt100; stty sane; exec {}", shell_command)])
            .stdin(Stdio::inherit())
                ...
                if let Some(signal) = exit_status.signal() {
                    match signal {
                        ...
                        _ => Err(VortexError::VmError {
                            message: format!("Interactive session terminated by signal: {}", signal),
                        })
                    }
                } else {
    ...
    async fn list_vms(&self) -> Result<Vec<String>> {
        ...
        let vm_names: Vec<String> = stdout
            ...
            .filter_map(|line| {
                let line = line.trim();
                if !line.is_empty() && !line.contains("CPUs:") && !line.contains("RAM") 
                   && !line.contains("DNS") && !line.contains("Buildah") 
                   && !line.contains("Workdir") && !line.contains("Mapped") {
                    Some(line.to_string())
                } else {
                    None
                }
