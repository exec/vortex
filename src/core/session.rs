...
pub enum SessionCommand {
    // Session management
    CreateSession { spec: VmSpec, name: Option<String>, persistent: bool },
    ListSessions,
    ...
    // Interactive
    AttachSession { session_id: String, client_pid: u32 },
...
pub enum SessionResponse {
    Success,
    Error { message: String },
    SessionCreated { session: VmSession },
    SessionList { sessions: Vec<VmSession> },
    Session { session: VmSession },
    DaemonStatus { 
        uptime: u64, 
...
impl SessionManager {
    ...
    async fn load_sessions(&mut self) -> Result<()> {
        ...
        let content = tokio::fs::read_to_string(&self.session_file).await.map_err(|e| {
            VortexError::VmError {
                message: format!("Failed to read sessions file: {}", e),
            }
        })?;
        
        let sessions: HashMap<String, VmSession> = serde_json::from_str(&content).unwrap_or_default();
    ...
    async fn save_sessions(&self) -> Result<()> {
        ...
        tokio::fs::write(&self.session_file, content).await.map_err(|e| {
            VortexError::VmError {
            ...
            match session.state {
                SessionState::Running | SessionState::Attached { .. } => {
                    if !running_vm_ids.contains(&session.vm_id) {
                        tracing::warn!("Session {} VM {} not found, marking as stopped", session.id, session.vm_id);
                        session.state = SessionState::Stopped;
                    }
                }
                SessionState::Stopped | SessionState::Error { .. } => {
                    if running_vm_ids.contains(&session.vm_id) {
                        tracing::info!("Session {} VM {} found running, updating state", session.id, session.vm_id);
    ...
    pub async fn create_session(&self, spec: VmSpec, name: Option<String>, persistent: bool) -> Result<VmSession> {
        ...
        vm_spec.labels.insert("session_id".to_string(), session_id.clone());
        vm_spec.labels.insert("persistent".to_string(), persistent.to_string());
        if let Some(ref name) = name {
            vm_spec.labels.insert("session_name".to_string(), name.clone());
        }
        ...
        match self.vm_manager.create(vm_spec).await {
            Ok(vm_instance) => {
                ...
                tracing::info!("Created session {} with VM {}", session_id, updated_session.vm_id);
                Ok(updated_session)
            }
            Err(e) => {
                let mut failed_session = session;
                failed_session.state = SessionState::Error { message: e.to_string() };
    ...
    pub async fn delete_session(&self, session_id: &str) -> Result<()> {
        ...
        if let Some(session) = session {
            ...
            if let Err(e) = self.vm_manager.cleanup(&session.vm_id).await {
                tracing::warn!("Failed to cleanup VM {} for session {}: {}", session.vm_id, session_id, e);
            }
    ...
    pub async fn start_session(&self, session_id: &str) -> Result<()> {
        ...
        match session.state {
            ...
            _ => {
                Err(VortexError::VmError {
                    message: format!("Cannot start session {} in state {:?}", session_id, session.state),
                })
            }
        }
    }
    ...
    pub async fn stop_session(&self, session_id: &str) -> Result<()> {
        ...
        if let Err(e) = self.vm_manager.stop(&session.vm_id).await {
            tracing::warn!("Failed to stop VM {} for session {}: {}", session.vm_id, session_id, e);
        }
    ...
    pub async fn attach_session(&self, session_id: &str, client_pid: u32) -> Result<()> {
        ...
        match session.state {
            ...
            _ => {
                Err(VortexError::VmError {
                    message: format!("Cannot attach to session {} in state {:?}", session_id, session.state),
                })
            }
        }
    }
    ...
    pub async fn get_daemon_status(&self) -> Result<SessionResponse> {
        ...
        let active_vms = sessions.values()
            .filter(|s| matches!(s.state, SessionState::Running | SessionState::Attached { .. } | SessionState::Detached))
            .count();
    ...
    pub async fn handle_command(&self, command: SessionCommand) -> Result<SessionResponse> {
        match command {
            ...
            SessionCommand::AttachSession { session_id, client_pid } => {
            ...
            for (session_id, session) in sessions.iter() {
                if !session.persistent {
                    ...
                    if let Some(last_attached) = session.last_attached {
                        ...
                        if hours_since_attach > 24 && matches!(session.state, SessionState::Detached) {
