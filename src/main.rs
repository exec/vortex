...
use std::sync::Arc;
use tracing::info;
use vortex_core::{VortexCore, VmSpec, ResourceLimits};
...
enum Commands {
    ...
    Run {
        ...
        #[arg(short = 'q', long, help = "Quiet mode - suppress all non-command output")]
        quiet: bool,
        
        #[arg(long, help = "Show real-time performance stats (Docker can't do this)")]
        monitor_performance: bool,
        
        #[arg(long, help = "Copy contents of host directory to VM directory (host:guest)")]
        copy_to: Vec<String>,
        
        #[arg(long, help = "Copy VM directory contents back to host after execution (guest:host)")]
        ...
        #[arg(long, help = "Cache dependencies for faster subsequent runs (Docker can't do this efficiently)")]
        cache_deps: bool,
    },
    ...
    Shell {
        #[arg(help = "VM image (alpine, ubuntu, node, etc.)", default_value = "alpine")]
        image: String,
...
async fn main() -> Result<()> {
    ...
    let vortex = Arc::new(vortex_core::init().await
    ...
    match cli.command {
        Commands::Run {
            ...
            run_vm(&vortex, spec, persist, run_quiet, monitor_performance, copy_to, sync_back, workdir, cache_deps).await?;
        }
        Commands::List => {
        ...
        Commands::Shell { 
            ...
            start_shell(&vortex, image, memory, cpus, port, volume, shell, quiet, copy_to, workdir).await?;
        }
        Commands::Metrics { vm_id } => {
            show_metrics(&vortex, vm_id.as_deref()).await?;
        }
        Commands::Parallel { images, command, quiet, copy_to, sync_back } => {
...
async fn run_vm(
    ...
    if cache_deps {
        let cache_dir = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string()) + "/.vortex/cache";
    ...
    for (i, (host_path, _)) in copy_mappings.iter().enumerate() {
        ...
        spec.volumes.insert(host_path.clone(), PathBuf::from(&temp_mount));
    }
    ...
    if let Some(original_cmd) = &spec.command {
        ...
        for (i, (_, dest_path)) in copy_mappings.iter().enumerate() {
            let temp_mount = format!("/tmp/vortex_copy_in_{}", i);
            enhanced_cmd.push_str(&format!("mkdir -p '{}' && cp -r {}/* '{}' 2>/dev/null || true; ", 
                                         dest_path.display(), temp_mount, dest_path.display()));
        }
        ...
        for (i, (source_path, _)) in sync_mappings.iter().enumerate() {
            let temp_mount = format!("/tmp/vortex_copy_out_{}", i);
            enhanced_cmd.push_str(&format!(" cp -r '{}' '{}' 2>/dev/null || true;", 
                                         source_path.display(), temp_mount));
        }
    ...
    if persist {
        if !quiet {
            info!("VM {} started and persisting. Use 'vortex stop {}' to stop it.", vm.id, vm.id);
        }
    } else {
        if !quiet {
...
async fn run_template(vortex: &Arc<VortexCore>, template_name: &str, override_command: Option<String>) -> Result<()> {
    let config = vortex_core::VortexConfig::load()?;
    let template = config.get_template(template_name)
        .ok_or_else(|| anyhow::anyhow!("Template '{}' not found", template_name))?;
    
    info!("Running template: {} - {}", template_name, template.description);
    ...
    run_vm(vortex, spec, false, false, false, vec![], vec![], None, false).await?;
    Ok(())
}
...
async fn show_templates() -> Result<()> {
    ...
    for (name, template) in &config.templates {
        ...
        if template.memory != 512 || template.cpus != 1 {
            println!("    Resources: {}MB RAM, {} CPU(s)", template.memory, template.cpus);
        }
        println!();
    }
...
async fn start_shell(
    ...
    for (i, (host_path, dest_path)) in copy_mappings.iter().enumerate() {
        ...
        shell_cmd.push_str(&format!("mkdir -p '{}' && cp -r {}/* '{}' 2>/dev/null || true; ", 
                                  dest_path.display(), temp_mount, dest_path.display()));
    }
    ...
    if !quiet {
        ...
        println!("│  Resources: {}MB RAM, {} CPU(s)", vm.spec.memory, vm.spec.cpus);
...
async fn show_metrics(vortex: &Arc<VortexCore>, vm_id: Option<&str>) -> Result<()> {
    if let Some(vm_id) = vm_id {
        ...
        if let Some(vm) = vms.iter().find(|v| v.id == vm_id) {
            match vm.backend.get_metrics(vm).await {
                Ok(metrics) => {
                    ...
                    println!("  Disk Usage: {:.1}MB", metrics.disk_usage as f64 / 1024.0 / 1024.0);
        ...
        println!("  Total Memory Used: {:.1}MB / {:.1}MB", 
            total_memory_used as f64 / 1024.0 / 1024.0,
            total_memory_allocated as f64 / 1024.0 / 1024.0);
        if successful_metrics > 0 {
            println!("  Average CPU per VM: {:.1}%", total_cpu_usage / successful_metrics as f64);
        }
    }
...
fn parse_port_mappings(ports: Vec<String>) -> Result<HashMap<u16, u16>> {
    ...
    for port in ports {
        ...
        if parts.len() != 2 {
            return Err(anyhow::anyhow!("Invalid port mapping format: {}. Use host:guest", port));
        }
        
        let host_port: u16 = parts[0].parse()
            .with_context(|| format!("Invalid host port: {}", parts[0]))?;
        let guest_port: u16 = parts[1].parse()
...
fn parse_volume_mappings(volumes: Vec<String>) -> Result<HashMap<PathBuf, PathBuf>> {
    ...
    for volume in volumes {
        ...
        if parts.len() != 2 {
            return Err(anyhow::anyhow!("Invalid volume mapping format: {}. Use host:guest", volume));
        }
...
fn parse_labels(labels: Vec<String>) -> Result<HashMap<String, String>> {
    ...
    for label in labels {
        ...
        if parts.len() != 2 {
            return Err(anyhow::anyhow!("Invalid label format: {}. Use key=value", label));
        }
...
fn parse_copy_mappings(copy_to: Vec<String>) -> Result<Vec<(PathBuf, PathBuf)>> {
    ...
    for copy_spec in copy_to {
        ...
        if parts.len() != 2 {
            return Err(anyhow::anyhow!("Invalid copy-to format: {}. Use host_dir:guest_dir", copy_spec));
        }
        ...
        if !host_path.exists() {
            return Err(anyhow::anyhow!("Host directory does not exist: {}", host_path.display()));
        }
        if !host_path.is_dir() {
            return Err(anyhow::anyhow!("Host path is not a directory: {}", host_path.display()));
        }
...
fn parse_sync_back_mappings(sync_back: Vec<String>) -> Result<Vec<(PathBuf, PathBuf)>> {
    ...
    for sync_spec in sync_back {
        ...
        if parts.len() != 2 {
            return Err(anyhow::anyhow!("Invalid sync-back format: {}. Use guest_dir:host_dir", sync_spec));
        }
...
async fn run_parallel_vms(
    ...
    if !quiet {
        println!("🚀 Launching {} VMs in parallel (try this with Docker!)", images.len());
    }
    ...
    for (i, image) in images.into_iter().enumerate() {
        ...
        let task = async move {
            ...
            run_vm(vortex, spec, false, true, false, copy_to, unique_sync_back, None, false).await?;
    ...
    if !quiet {
        ...
        println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        println!("🚀 Total time: {:.2}s ({} VMs in parallel)", total_duration.as_secs_f64(), results.len());
        println!("⚡ Docker would take {}x longer running these sequentially!", results.len());
    }
...
🎯 Real-time Performance Monitor (Docker can't do this!)");
    ...
    for i in 1..=10 {
        ...
        if let Ok(vms) = vortex.vm_manager.list().await {
            if let Some(vm) = vms.iter().find(|v| v.id == vm_id) {
                if let Ok(metrics) = vm.backend.get_metrics(vm).await {
                    print!("\r[{}s] CPU: {:.1}% | RAM: {:.0}MB/{:.0}MB | Disk: {:.0}MB", 
                          i,
                          metrics.cpu_usage,
                          metrics.memory_usage as f64 / 1024.0 / 1024.0,
                          metrics.memory_total as f64 / 1024.0 / 1024.0,
